...
ScanP='/media/deni/c78a9273-3214-4387-9f72-4cdc3adef255/OnBP_9Aug18/DeeperRegion/';
RefFldMapP='/media/deni/c78a9273-3214-4387-9f72-4cdc3adef255/OnBP_9Aug18/DeeperRegion/meas_MID507_BP_fieldmap_5echosX_FID26075/';
BaseFN='meas_MID526_gBP_ep2d_bold_multiecho_ASL_SMS_Spic_OutInOut3ADCs_U3_FID26094';
...
gStruct2txt(SummaryStr,[ScanP BaseFN filesep 'Summary.txt']);
%%
...
title(['MaxK=' num2str(MaxK) ' #Traj=' num2str(nTraj) ' Acc=' num2str(Acc) ' VD=' num2str(VD)]);
subplot(2,2,2);
...
% for nTS=8:15
    ...
    TSC(:,:,:,CurB)=exp(1i*2*pi*RepDotMult(B0M2(:,:,CurB),gpermute(FesTimePoints,[3 2])));  % exp(1i*2*pi*(TimeInMs2/1000)*B0M2);
    TSC2=reshape(TSC,prod(Sz2),nTS);
    WTSC2=WE.*TSC2;
%     tic
%     TSB=(E.')/(TSC2.');% W in both sides
    TSB(:,:,CurB)=(WeightedE.')/(WTSC2.');% W in both sides
...
% for nTS=8:15
    ...
    FesTimePoints=linspace(0,TimeInMs2(end)/1000,nTS);
    TSC(:,:,:,CurB)=exp(1i*2*pi*RepDotMult(B0M2(:,:,CurB),gpermute(FesTimePoints,[3 2])));  % exp(1i*2*pi*(TimeInMs2/1000)*B0M2);
    TSC2=reshape(TSC(:,:,:,CurB),prod(Sz2),nTS);
    WTSC2=WE.*TSC2;
%     tic
%     TSB=(E.')/(TSC2.');% W in both sides
    TSB(:,:,CurB)=(WeightedE.')/(WTSC2.');% W in both sides
...
%     ErrTS(nTS)=grmss(E-TSC2*(TSB.')); %include W
    ErrTS(nTS)=grmss(WeightedE-WTSC2*(TSB(:,:,CurB).')); %include W
